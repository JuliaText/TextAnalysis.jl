@testset "Custom layers" begin
    ...
    @testset "AWD_LSTM" begin
        awd = AWD_LSTM(4, 3, 0.3)
        @test typeof(awd.layer.cell) == WeightDroppedLSTMCell
        @test awd.accum isa undef
        set_Trigger!(4, awd)
        @test awd.T == 4
        asgd_step!(4, awd)
...
@testset "Language model"
    ...
    @test length(get_trainable_params(lm.layers)) == 10

    pretrained_weights = BSON.load(datadep"Pretrained ULMFiT Language Model")
    @test length(pretrained_weights[:weights]) == 16
    @test all(size.(params(lm)) .== size.(weights))
end
